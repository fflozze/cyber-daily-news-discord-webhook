...
SOURCE_LANGS = os.getenv("SOURCE_LANGS", "fr,en")  # << chercher en FR + EN
TZ           = os.getenv("TIMEZONE", "Europe/Paris")
COLOR        = int(os.getenv("EMBED_COLOR", "15158332"))  # 0xE74C3C (rouge)
...
def chunk_text(txt: str, size: int = FIELD_SOFT_MAX):
    """Split by lines, fallback to hard-chunk for very long lines (e.g., URLs)."""
    txt = (txt or "").strip()
    if not txt:
        return []
...
def post_discord_embeds(title: str, description: str, bullet_text: str, links_text: str):
    ...
    first_embed = {
        ...
        "color": COLOR,
        "footer": {"text": f"Veille Cyber • {date_str} • Window: {HOURS}h • {TZ}"},
    }
...
def build_prompt():
    """
    CYBER NEWS scope:
      - Security advisories (CVE), zero-days, exploits, PoCs, patches
      - Ransomware, data breaches, APT/nation-state, threat intel & IOCs
      - Vendor PSIRTs, cloud security, identity & access (IAM), MFA
      - Blue/Red/Purple team tooling, SIEM/SOAR/EDR/XDR, OT/ICS/SCADA
      - Regulations & frameworks (NIST CSF, ISO 27001, SOC 2, DORA, NIS2)
      - Supply-chain risks (npm/pypi), bug bounties, incident response
    """
    langs_note = SOURCE_LANGS.replace(",", ", ")
    return textwrap.dedent(f"""
    Tu es un analyste de **cybersécurité** chargé d'une veille quotidienne.

    Tâche :
    - Utilise la **recherche web** pour identifier les **actualités cybersécurité** publiées entre
      **{since_utc.strftime("%Y-%m-%d %H:%M UTC")}** et **{now_utc.strftime("%Y-%m-%d %H:%M UTC")}**.
    - **Sources à considérer** : en **français et en anglais** ({langs_note}). Priorise la **fiabilité** (PSIRT éditeurs,
      CERT/CISA/ANSSI, .gov/.edu, billets de recherche sérieux). Si une info n’existe qu’en anglais, **inclue-la**.
      Préfère la **source primaire** (advisory initial) et **évite les doublons** d’un même événement.

    Rendu en **français ({LOCALE})** et en **Markdown** :
    1) Un titre : "Veille Cyber — {date_str} (dernières {HOURS} h)".
    2) Un **résumé global** (2–3 phrases).
    3) **5–10 puces** factuelles (CVE/ID, CVSS si dispo, versions corrigées, impact, acteurs, régions).
    4) Une section **Liens / Links** (10–15 items max) au format :
       - **[FR]** ou **[EN]** — Titre court — URL (et la **date/heure** si dispo).
...
def main():
    ...
    try:
        ...
        post_discord_embeds(
            title=f"Veille Cyber — {date_str} (dernières {HOURS} h)",
            description=err,
            bullet_text="",
            links_text=""
        )
    ...
    fallback_title = f"Veille Cyber — {date_str} (dernières {HOURS} h)"
